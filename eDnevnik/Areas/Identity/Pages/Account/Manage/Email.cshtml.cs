...
namespace eDnevnik.Areas.Identity.Pages.Account.Manage
{
    public class EmailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostSendVerificationEmailAsync()
        {
            ...
            {
                StatusMessage = "Došlo je do greške prilikom promjene email adrese.";
                return RedirectToPage();
            }

            // Ako želiš da i UserName bude isti kao Email (po defaultu jeste kod većine):
            var setUserNameResult = await _userManager.SetUserNameAsync(user, Input.NewEmail);
            if (!setUserNameResult.Succeeded)
            {
                StatusMessage = "Došlo je do greške prilikom promjene korisničkog imena.";
                return RedirectToPage();
            }

            await _signInManager.RefreshSignInAsync(user);
            StatusMessage = "Email adresa je uspješno promijenjena.";
            return RedirectToPage();
